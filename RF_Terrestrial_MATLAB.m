clc;
clear;
close all;

c = physconst('lightspeed');
f_GHz = 40 : 0.1 : 60; % Freqency range over which antennas are operating (40-60GHZ)
lambda = c ./ (f_GHz * 1e9); % Wavelength in [m]
D = 0.15; % [m], diameter of the antenna 
d_m=200; % [m] distance between two antennas

%% Calculation of antenna gain according to ITU-R F.699-9 exactly at 50GHz 
D_lam = D ./ lambda;
Gmax = 20*log10(D_lam) + 7.7; % [dBi] the main lobe antenna gain 
G1 = 2 + 15*log10(D_lam); % [dBi] gain of the first side-lobe

f_spot   = 50;                   % [GHz] Frequency, where the Gain is calculated
idx      = find(abs(f_GHz - f_spot) < 0.06, 1);
% Extract scalars at the chosen frequency
D_lam_spot  = D_lam(idx);
Gmax_spot   = Gmax(idx);
G1_spot     = G1(idx);
phi_m_spot  = (20 / D_lam_spot) * sqrt(Gmax_spot - G1_spot);  % scalar
phi_b_spot  = 100 / D_lam_spot;                           % scalar
    

phi = 0.1:0.1:180;
G   = zeros(size(phi));

for k = 1:length(phi)
    p = phi(k);
    if p < phi_m_spot
        G(k) = Gmax_spot - 2.5e-3 * (D_lam_spot * p)^2;
    elseif p < phi_b_spot
        G(k) = G1_spot;
    elseif p < 48
        G(k) = 52 - 10*log10(D_lam_spot) - 25*log10(p);
    else
        G(k) = 10 - 10*log10(D_lam_spot);
    end
end





%% Free Space Path Loss
d_km = d_m / 1000;    % path length [km]
f_MHz = f_GHz * 1000;
% Free-space path loss — ITU-R P.525-4
L_fs = 32.4 + 20*log10(f_MHz) + 20*log10(d_km);  


%% gas attenuation
T_K     = 284.15;        % temperature [K]  (11 C)
P_hPa   = 1016;       % pressure [hPa]
rho     = 7.5;        % water vapour density [g/m3]

gamma_O2  = zeros(size(f_GHz));
gamma_H2O = zeros(size(f_GHz));
for k = 1:length(f_GHz)
    [gamma_O2(k), gamma_H2O(k)] = p676_annex1(f_GHz(k), T_K, P_hPa, rho);
end
L_gas = (gamma_O2 + gamma_H2O) * d_km;

%% rain attenuation model 



%%FIGURES
% antenna gain graph for 0-30 degrees
figure(1); 
plot(phi, G, 'b-', 'LineWidth', 2);
hold on;
xline(phi_m_spot, 'r--',  'Label', sprintf('\\phi_m = %.2f°', phi_m_spot), 'LabelVerticalAlignment', 'bottom','LabelHorizontalAlignment', 'left');
xline(phi_b_spot, 'k--', 'Label', sprintf('\\phi_b = %.2f°', phi_b_spot), 'LabelVerticalAlignment', 'bottom');
yline(G1_spot, 'g--', 'Label', sprintf('G1 = %.1f dBi', G1_spot));
yline(Gmax_spot, '--','Color', '#800080', 'Label', sprintf('Gmax = %.1f dBi', Gmax_spot));
hold off;
xlabel('Off-axis angle [degrees]');
ylabel('Gain [dBi]');
title('Antenna radiation pattern ITU-R F.699-9');
grid on;
xlim([0 30]);

%Free-space path loss vs frequency graph
figure(2);
plot(f_GHz, L_fs, 'b-', 'LineWidth', 2);
xlabel('Frequency [GHz]');
ylabel('Path loss [dB]');
title('Free-space path loss — ITU-R P.525-5');
grid on;

% gas attanuation graph
figure(3);
plot(f_GHz, L_gas, 'k-', 'LineWidth', 2);
hold on;
plot(f_GHz, gamma_O2  * d_km, 'r-',  'LineWidth', 1.5, 'DisplayName', 'O2');
plot(f_GHz, gamma_H2O * d_km, 'b--', 'LineWidth', 1.5, 'DisplayName', 'H2O');
hold off;
xlabel('Frequency [GHz]');
ylabel('Gas attenuation [dB]');
title(sprintf('Gas attenuation over %.0f m path — ITU-R P.676-13 Annex 1', d_m));
legend('Total', 'O2', 'H2O');
grid on;
xlim([40 60]);




%% FUNCTIONS
function [gamma_O2, gamma_H2O] = p676_annex1(f, T_K, P_hPa, rho)

% Spectroscopic data — Table 1: Oxygen
% Columns: f0, a1, a2, a3, a4, a5, a6
O2 = [
  50.474214,  0.975, 9.651,  6.690, 0.0,  2.566,  6.850;
  50.987745,  2.529, 8.653,  7.170, 0.0,  2.246,  6.800;
  51.503360,  6.193, 7.709,  7.640, 0.0,  1.947,  6.729;
  52.021429, 14.320, 6.819,  8.110, 0.0,  1.667,  6.640;
  52.542418, 31.240, 5.983,  8.580, 0.0,  1.388,  6.526;
  53.066934, 64.290, 5.201,  9.060, 0.0,  1.349,  6.206;
  53.595775,124.600, 4.474,  9.550, 0.0,  2.227,  5.085;
  54.130025,227.300, 3.800,  9.960, 0.0,  3.170,  3.750;
  54.671180,389.700, 3.182, 10.370, 0.0,  3.558,  2.654;
  55.221384,627.100, 2.618, 10.890, 0.0,  2.560,  2.952;
  55.783815,945.300, 2.109, 11.340, 0.0, -1.172,  6.135;
  56.264774,543.400, 0.014, 17.030, 0.0,  3.525, -0.978;
  56.363399,1331.800,1.654, 11.890, 0.0, -2.378,  6.547;
  56.968211,1746.600,1.255, 12.230, 0.0, -3.545,  6.451;
  57.612486,2120.100,0.910, 12.620, 0.0, -5.416,  6.056;
  58.323877,2363.700,0.621, 12.950, 0.0, -1.932,  0.436;
  58.446588,1442.100,0.083, 14.910, 0.0,  6.768, -1.273;
  59.164204,2379.900,0.387, 13.530, 0.0, -6.561,  2.309;
  59.590983,2090.700,0.207, 14.080, 0.0,  6.957, -0.776;
  60.306056,2103.400,0.207, 14.150, 0.0, -6.395,  0.699;
  60.434778,2438.000,0.386, 13.390, 0.0,  6.342, -2.825;
  61.150562,2479.500,0.621, 12.920, 0.0,  1.014, -0.584;
  61.800158,2275.900,0.910, 12.630, 0.0,  5.014, -6.619;
  62.411220,1915.400,1.255, 12.170, 0.0,  3.029, -6.759;
  62.486253,1503.000,0.083, 15.130, 0.0, -4.499,  0.844;
  62.997984,1490.200,1.654, 11.740, 0.0,  1.856, -6.675;
  63.568526,1078.000,2.108, 11.340, 0.0,  0.658, -6.139;
  64.127775, 728.700,2.617, 10.880, 0.0, -3.036, -2.895;
  64.678910, 461.300,3.181, 10.380, 0.0, -3.968, -2.590;
  65.224078, 274.000,3.800,  9.960, 0.0, -3.528, -3.680;
  65.764779, 153.000,4.473,  9.550, 0.0, -2.548, -5.002;
  66.302096,  80.400,5.200,  9.060, 0.0, -1.660, -6.091;
  66.836834,  39.800,5.982,  8.580, 0.0, -1.680, -6.393;
  67.369601,  18.560,6.818,  8.110, 0.0, -1.956, -6.475;
  67.900868,   8.172,7.708,  7.640, 0.0, -2.216, -6.545;
  68.431006,   3.397,8.652,  7.170, 0.0, -2.492, -6.600;
  68.960312,   1.334,9.650,  6.690, 0.0, -2.773, -6.650;
 118.750334, 940.300,0.010, 16.640, 0.0, -0.439,  0.079;
 368.498246,  67.400,0.048, 16.400, 0.0,  0.000,  0.000;
 424.763020, 637.700,0.044, 16.400, 0.0,  0.000,  0.000;
 487.249273, 237.400,0.049, 16.000, 0.0,  0.000,  0.000;
 715.392902,  98.100,0.145, 16.000, 0.0,  0.000,  0.000;
 773.839490, 572.300,0.141, 16.200, 0.0,  0.000,  0.000;
 834.145546, 183.100,0.145, 14.700, 0.0,  0.000,  0.000];

% Spectroscopic data — Table 2: Water vapour
% Columns: f0, b1, b2, b3, b4, b5, b6
H2O = [
   22.235080, 0.1079, 2.144, 26.38, 0.76, 5.087, 1.00;
   67.803960, 0.0011, 8.732, 28.58, 0.69, 4.930, 0.82;
  119.995940, 0.0007, 8.353, 29.48, 0.70, 4.780, 0.79;
  183.310087, 2.2730, 0.668, 29.06, 0.77, 5.022, 0.85;
  321.225630, 0.0470, 6.179, 24.04, 0.67, 4.398, 0.54;
  325.152888, 1.5140, 1.541, 28.23, 0.64, 4.893, 0.74;
  336.227764, 0.0010, 9.825, 26.93, 0.69, 4.740, 0.61;
  380.197353,11.6700, 1.048, 28.11, 0.54, 5.063, 0.89;
  390.134508, 0.0045, 7.347, 21.52, 0.63, 4.810, 0.55;
  437.346667, 0.0632, 5.048, 18.45, 0.60, 4.230, 0.48;
  439.150807, 0.9098, 3.595, 20.07, 0.63, 4.483, 0.52;
  443.018343, 0.1920, 5.048, 15.55, 0.60, 5.083, 0.50;
  448.001085,10.4100, 1.405, 25.64, 0.66, 5.028, 0.67;
  470.888999, 0.3254, 3.597, 21.34, 0.66, 4.506, 0.65;
  474.689092, 1.2600, 2.379, 23.20, 0.65, 4.804, 0.64;
  488.490108, 0.2529, 2.852, 25.86, 0.69, 5.201, 0.72;
  503.568532, 0.0372, 6.731, 16.12, 0.61, 3.980, 0.43;
  504.482692, 0.0124, 6.731, 16.12, 0.61, 4.010, 0.45;
  547.676440, 0.9785, 0.158, 26.00, 0.70, 4.500, 1.00;
  552.020960, 0.1840, 0.158, 26.00, 0.70, 4.500, 1.00;
  556.935985,497.000, 0.159, 30.86, 0.69, 4.552, 1.00;
  620.700807, 5.0150, 2.391, 24.38, 0.71, 4.856, 0.68;
  645.766085, 0.0067, 8.633, 18.00, 0.60, 4.000, 0.50;
  658.005280, 0.2732, 7.816, 32.10, 0.69, 4.140, 1.00;
  752.033113,243.400, 0.396, 30.86, 0.68, 4.352, 0.84;
  841.051732, 0.0134, 8.177, 15.90, 0.33, 5.760, 0.45;
  859.965698, 0.1325, 8.055, 30.60, 0.68, 4.090, 0.84;
  899.303175, 0.0547, 7.914, 29.85, 0.68, 4.530, 0.90;
  902.611085, 0.0386, 8.429, 28.65, 0.70, 5.100, 0.95;
  906.205957, 0.1836, 5.110, 24.08, 0.70, 4.700, 0.53;
  916.171582, 8.4000, 1.441, 26.73, 0.70, 5.150, 0.78;
  923.112692, 0.0079,10.293, 29.00, 0.70, 5.000, 0.80;
  970.315022, 9.0090, 1.919, 25.50, 0.64, 4.940, 0.67;
  987.926764,134.600, 0.257, 29.85, 0.68, 4.550, 0.90;
 1780.000000,17506.0, 0.952,196.30, 2.00,24.150, 5.00];

theta = 300 / T_K;% This is the normalised inverse temperature
e = rho * T_K / 216.7; % Water vapour partial pressure [hPa]
p = P_hPa - e; % Dry air presure


% Oxygen N''
N_O2 = 0;                    % Σ starts at 0
for i = 1:size(O2, 1)
    f0 = O2(i,1);   % line centre frequency [GHz]
    a1 = O2(i,2);
    a2 = O2(i,3);
    a3 = O2(i,4);
    a4 = O2(i,5);   % always 0.0 for oxygen in Table 1
    a5 = O2(i,6);
    a6 = O2(i,7);

    Si = a1 * 1e-7 * p * theta^3 * exp(a2 * (1 - theta));  % Oxygen line strength Si — Equation (3)
    Df = a3 * 1e-4 * (p * theta^(0.8 - a4) + 1.1 * e * theta); % Line width Δf — Equation (6a), oxygen
    Df = sqrt(Df^2 + 2.25e-6); % Zeeman splitting correction — Equation (6b), oxygen
    delta = (a5 + a6 * theta) * 1e-4 * (p + e) * theta^0.8; % Interference correction factor δ — Equation (7), oxygen
 
    Fi = (f / f0) * ((Df - delta*(f0 - f)) / ((f0 - f)^2 + Df^2) + (Df - delta*(f0 + f)) / ((f0 + f)^2 + Df^2) );% Line shape factor Fi — Equation (5)
    N_O2 = N_O2 + Si * Fi;
end
d_deb = 5.6e-4 * (p + e) * theta^0.8; % d_deb is the width parameter for the Debye spectrum

N_D = f * p * theta^2 * (6.14e-5 / (d_deb * (1 + (f/d_deb)^2)) + 1.4e-12 * p * theta^1.5 / (1 + 1.9e-5 * f^1.5) ); % Add dry continuum — Equation (8)
N_O2 = N_O2 + N_D;

% Specific oxygen attenuation — Equation (1)
gamma_O2 = 0.1820 * f * N_O2;   % [dB/km]
gamma_O2 = max(gamma_O2, 0);

% Water vapour N'' — Equations (2b), (3), (5), (6)
N_H2O = 0;

for i = 1:size(H2O, 1)
    f0 = H2O(i,1);
    b1 = H2O(i,2);
    b2 = H2O(i,3);
    b3 = H2O(i,4);
    b4 = H2O(i,5);
    b5 = H2O(i,6);
    b6 = H2O(i,7);

    Si = b1 * 1e-1 * e * theta^3.5 * exp(b2 * (1 - theta)); % Line strength Si for watr
    Df = b3 * 1e-4 * (p * theta^b4 + b5 * e * theta^b6);  % Line width Δf water vapour (6a)
    Df = 0.535 * Df + sqrt(0.217 * Df^2 + (1.2316e-12 * f0^2 / theta)); %(6b)
    delta = 0;
    Fi = (f / f0) * ((Df - delta*(f0 - f)) / ((f0 - f)^2 + Df^2) + (Df - delta*(f0 + f)) / ((f0 + f)^2 + Df^2) );
    N_H2O = N_H2O + Si * Fi;
end

gamma_H2O = 0.1820 * f * N_H2O;   % [dB/km]
gamma_H2O = max(gamma_H2O, 0);

end 

